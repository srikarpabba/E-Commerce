...
export class BasketService {
  ...
  setBasket(basket: IBasket): any {
    // we want this to execute cos its return a new basket
  ...
  removeItemFromBasket(item: IBasketItem): void {
    ...
    if (basket.items.some((x) => x.id === item.id)) {
      basket.items = basket.items.filter((itm) => itm.id !== item.id);
  ...
  // deletes the basket from client - after a succesful order
